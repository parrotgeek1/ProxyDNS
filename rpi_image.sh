...
cd sdcard
rm -rf modules overlays vc *.symvers start{4,_,}{,db,x}.elf git_hash *.md fixup{4,_,}{,db,x}.dat uname_string* bcm2708-rpi-zero*.dtb bcm2708-rpi-0*.dtb bcm*-rpi-cm*.dtb kernel8.img
cat << EOF > config.txt
gpu_mem=16
boot_delay=0
disable_splash=1
